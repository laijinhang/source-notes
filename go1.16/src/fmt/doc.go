...
/*
	软件包fmt使用实现格式化的I/O类似C的printf和scanf。格式“verbs”是从c派生的，但是比较简单
	...
	浮点型 and complex constituents:
		...
		%F	是％f的同义词
		%g	对于大指数，％g％e，否则为％f。精度将在下面讨论。
		%G	％E适用于大指数，否则为％F
		%x	十六进制表示法（具有两个指数的十进制幂），例如-0x1.23abcp + 20
		%X	大写十六进制表示法，例如-0X1.23ABCP + 20
	字符串和字节切片 (treated equivalently with these verbs):
		%s	字符串或切片的未解释字节
		%q	用Go语法安全地转义的双引号字符串
		%x	以16为基数，小写，每字节两个字符
		%X	以16为基数，大写，每字节两个字符
