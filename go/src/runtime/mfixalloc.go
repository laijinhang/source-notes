...
type fixalloc struct {
	size   uintptr
	first  func(arg, p unsafe.Pointer) // called first time p is returned	// 首次调用时，返回p
	arg    unsafe.Pointer
	list   *mlink
	chunk  uintptr // use uintptr instead of unsafe.Pointer to avoid write barriers	// 使用uintptr而非unsafe.Pointer来避免write barriers
	nchunk uint32
