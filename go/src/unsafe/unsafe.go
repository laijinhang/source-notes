...
// 说明：这个文件里的中文翻译参考自，[go unsafe 注释翻译和实践](http://qqailab.cn/2019/06/16/go-package-unsafe.html)
/*
	...
	unsafe包包含一些可以绕过Go语言中的类型安全的操作，
	如果引用了unsafe包可能会导致不稳定，而且也不会受
	Go 1的兼容性所保护
*/
package unsafe
...
// 这里的ArbitraryType仅仅是为了文档的目的，实际上它不是unsafe包的一部分。它可以表示任何go表达式的类型
...
// Pointer表示指向任意类型的指针。Pointer有四种特殊操作，这四种特殊操作不适用于其他类型：
...
// 允许 go vet可以帮助找到不符合这些模式的Pointer用法，但是go vet的沉默并不能保证代码是有效的。
//
...
// (2) Pointer转换为uintptr，但不转换会Pointer
//
// Converting a Pointer to a uintptr produces the memory address of the value
// pointed at, as an integer. The usual use for such a uintptr is to print it.
// 将Pointer转换为uintper会产生指针指向内存地址，作为整数。
// uintptr的通常用处是打印它。
//
...
// 剩下的模式枚举了uintptr或Pointer的有效转换。
//
// (3) Conversion of a Pointer to a uintptr and back, with arithmetic.
// (3) Pointer转换为uintptr，计算之后再转换回来
//
...
// 通过这种方式为指针增加或减去偏移量都是有效的。
// 使用&^来四舍五入指针也是有效的，此操作通常用来做对齐。
// 所有情况下，结果必须继续指向原始分配的对象。
//
...
// (4) 调用syscall.Syscall时Pointer转换为uintptr
//
...
// 返回结构体内某个字段的偏移量，参数x必须是struct Value.field这种格式，
// 也就是说，Offsetof返回了开始位置和某个字段开始位置的偏移量，单位字节
