...
// 为了装饰日志信息而跳过辅助函数时要经过的最大堆栈帧数。
const maxStackLen = 50

// common holds the elements common between T and B and
// captures common methods such as Errorf.
// common保存了T和B之间的共同元素，并捕获了共同的方法，如Errorf。
type common struct {
	mu          sync.RWMutex         // guards this group of fields								// 保护这些字段
	output      []byte               // Output generated by test or benchmark.					// 由测试或基准产生的输出。
	w           io.Writer            // For flushToParent.										// 对于flushToParent。
	ran         bool                 // Test or benchmark (or one of its subtests) was executed.// 测试或基准（或其子测试之一）被执行。
	failed      bool                 // Test or benchmark has failed.							// 测试或基准失败。
	skipped     bool                 // Test or benchmark has been skipped.						// 测试或基准已被跳过。
	done        bool                 // Test is finished and all subtests have completed.		// 测试结束，所有分项测试都已完成。
	helperPCs   map[uintptr]struct{} // functions to be skipped when writing file/line info		// 写入文件/行信息时要跳过的功能
	helperNames map[string]struct{}  // helperPCs converted to function names					// 转换为函数名称的helperPCs
	cleanups    []func()             // optional functions to be called at the end of the test	// 测试结束时要调用的可选函数
	cleanupName string               // Name of the cleanup function.							// 清理功能的名称
	cleanupPc   []uintptr            // The stack trace at the point where Cleanup was called.	// 在调用Cleanup时的堆栈跟踪。
	finished    bool                 // Test function has completed.							// 测试功能已经完成。

	chatty     *chattyPrinter // A copy of chattyPrinter, if the chatty flag is set.			// ChattyPrinter的副本，如果设置了chatty标志。
	bench      bool           // Whether the current test is a benchmark.						// 目前的测试是否是一个基准
	hasSub     int32          // Written atomically.											// 以原子方式进行写操作
	raceErrors int            // Number of races detected during test.							// 测试过程中检测到的races数量
	runner     string         // Function name of tRunner running the test.						// 运行测试的tRunner的功能名称s

	parent   *common
	level    int       // Nesting depth of test or benchmark.										// 测试或基准的嵌套深度。
	creator  []uintptr // If level > 0, the stack trace at the point where the parent called t.Run.	// 如果级别>0，则在父类调用t.Run的地方出现堆栈跟踪。
	name     string    // Name of test or benchmark.												// 测试或基准的名称
	start    time.Time // Time test or benchmark started											// 测试或基准开始时间
	duration time.Duration
	barrier  chan bool // To signal parallel subtests they may start.	// 为了示意他们可以开始进行平行分测试。
	signal   chan bool // To signal a test is done.						// 发出测试完成的信号
	sub      []*T      // Queue of subtests to be run in parallel.		// 要并行运行的子测试队列。
...
// TB是T和B的公共接口，T和B都是需要实现TB的，这个接口定义下面有两行代码，专门用来校验T和B有没有实现TB接口
type TB interface {
	Cleanup(func())
	...
	// 一个私有方法，以防止用户实现该接口，因此未来对它的添加不会违反Go 1的兼容性。
...
/*
空变量，下面两行代码，用于校验 T 和 B 结构体有没有实现TB接口的定义
*/
var _ TB = (*T)(nil)
var _ TB = (*B)(nil)
...
// Test function.
// 当测试函数返回或调用FailNow、Fatal、Fatalf、SkipNow、Skip或Skipf中的任何一个方法时，
// 测试结束。这些方法以及Parallel方法，必须只从运行Test函数的goroutine中调用。
//
